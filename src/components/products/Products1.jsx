...
export default function Products1({ parentClass = "flat-spacing", collection }) {
      ...
      try {
        ...
        if (collection) {
          const filteredProducts = parsedProducts.filter((item) =>
            item.s_text.includes(collection)
          );
          setProductMain(filteredProducts);
        } else {
          setProductMain(parsedProducts);
        }
      } catch (error) {
  ...
  const allProps = {
    ...
    setCollection: (newCollection) => {
      const updated = [...collections].includes(newCollection)
        ? [...collections].filter((elm) => elm != newCollection)
        : [...collections, newCollection];
      dispatch({ type: "SET_COLLECTION", payload: updated });
    },
    setBrands: (newBrand) => {
    ...
    removeCollection: (newBrand) => {
      const updated = [...collections].filter((brand) => collection != newBrand);
      dispatch({ type: "SET_COLLECTION", payload: updated });
    },
  ...
  useEffect(() => {
    ...
    if (collections.length) {
      ...
      const filteredByCollection = [...productMain].filter((elm) =>
        collections.every((el) => elm.tabFilterOptions.includes(el))
      );
      filteredArrays = [...filteredArrays, filteredByCollection];
    }
    if (availability !== "All") {
  ...
  }, [price, availability, color, size, brands, activeFilterOnSale, productMain]);
  ...
  useEffect(() => {
    ...
    dispatch({ type: "SET_CURRENT_PAGE", payload: 1 });
  }, [filtered, sortingOption, productMain]);
  return (
    <>
